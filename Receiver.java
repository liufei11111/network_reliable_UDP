...
public class Receiver {
	...
	private static int StillNotReceiveFirstACK = -99;// the ACK for first packet
	private static boolean Debug_Mode = false;// if Debug mode
	public Receiver(){
	...
	public  void main_non_static(String args[]) {
		try {
			...
			if(Debug_Mode)
			System.err.println("UDP_receive() error!");
			//SEND_ENDING_ACK();
		} finally {
			//SEND_ENDING_ACK();
			if (LAST_PACKET)
	...
	public static int client_listen() {
		...
		try {
			// get things to show
			String serverHostname = TCP_ip_address;// get String IP
			...
			DatagramPacket receivePacket = new DatagramPacket(receiveData,
					receiveData.length);
			// this is the loop to process packages that comes from sender
			while (true) {
					try {
	...
	/**
	 * process single incoming package
	 * **/
	public static PacketInfo single_package(DatagramSocket clientSocket,
		...
		int length_true_data = receivePacket.getLength();// my buffer is always
															// bigger than this
		// use all get functions to have the attributes recovered from the packet
	...
	/**
	 * reasseble the bytes received into a file
	 * **/
	...
	/**
	 * return if the fin flag is received
	 * **/
	...
	/**
	 * return if the ACK flag is received
	 * **/
	...
	/**
	 * return  if the ACK num is received
	 * **/
